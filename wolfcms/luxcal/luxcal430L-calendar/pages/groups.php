...
function showGroups() {
	...
	echo "<table class='list'>
		<tr><th>&nbsp;{$ax['id']}&nbsp;</th><th>{$ax['grp_name']}</th><th>{$ax['grp_privs']}</th><th>{$ax['grp_categories']}</th><th>{$ax['grp_rep_events']}</th><th>{$ax['grp_m-d_events']}</th><th>{$ax['grp_priv_events']}</th><th></th></tr>\n";
	while ($row = $stH->fetch(PDO::FETCH_ASSOC)) {
...
function editGroup(&$group) {
	...
	echo "</td></tr>\n";
	echo "<tr><td><label for='rev'>{$ax['grp_may_post_revents']}</label>:</td><td><input type='checkbox' name='rEvts' id='chb' value='1'".($group['rEvts'] ? " checked='checked'" : '').">";
	echo "<tr><td><label for='rev'>{$ax['grp_may_post_mevents']}</label>:</td><td><input type='checkbox' name='mEvts' id='chb' value='1'".($group['mEvts'] ? " checked='checked'" : '').">";
	echo "<tr><td><label for='rev'>{$ax['grp_may_post_pevents']}</label>:</td><td><input type='checkbox' name='pEvts' id='chb' value='1'".($group['pEvts'] ? " checked='checked'" : '').">";
...
function addGroup(&$group) { //add group
	...
	do {
		...
		$stH = stPrep("INSERT INTO `groups` (`name`,`privs`,`catIDs`,`rEvents`,`mEvents`,`pEvents`,`color`) VALUES (?,?,?,?,?,?,?)");
		$result = stExec($stH,array($group['name'],$group['privs'],implode(',',$group['catIDs']),$group['rEvts'],$group['mEvts'],$group['pEvts'],$group['color']));
...
function updateGroup($group) { //update group
	...
	do {
		if ($group['color'] and !preg_match("/^#[0-9A-Fa-f]{6}$/", $group['color'])) { $msg = $ax['grp_invalid_color']; break; }
		if (!preg_match("/^[\w\s\._-]{2,}$/u", $group['name'])) { $msg = $ax['grp_name_invalid']; break; }
		$stH = stPrep("UPDATE `groups` SET `name` = ?,`privs` = ?,`catIDs` = ?,`rEvents` = ?,`mEvents` = ?,`pEvents` = ?,`color` = ? WHERE `ID` = ?");
		$result = stExec($stH,array($group['name'],$group['privs'],implode(',',$group['catIDs']),$group['rEvts'],$group['mEvts'],$group['pEvts'],$group['color'], $group['id']));
