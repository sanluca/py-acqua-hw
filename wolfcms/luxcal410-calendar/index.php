...
//error_reporting(E_ALL); //errors and notices - test
...
//load config data
if (!file_exists('./lcconfig.php')) {//no config data: install
...
if (!isset($_GET['lc'])) { //external hit
	...
	//connect to db
	$dbH = dbConnect($calID);
	if ($dbH === false or substr(LCV,0,6) != $lcV) { //LCV not current: upgrade
	...
	if (isset($_SESSION['lcUser'])) {
		...
		stExec($stH,array($_SESSION['lcUser']));
...
if ($set['restLastSel']) { loadLastSel($calID); }
...
$stH = stPrep("SELECT u.ID, u.name, u.email, g.privs, g.catIDs FROM users AS u INNER JOIN groups AS g ON g.ID = u.groupID WHERE u.ID = 1 OR u.ID = ? ORDER BY u.ID DESC"); //if userID not found, revert to public user
stExec($stH,array($_SESSION['uid']));
$row = $stH->fetch(PDO::FETCH_NUM);
$stH = null; //release statement handle!
list($uid,$uname,$umail,$privs,$ucats) = $row;
...
if ((isset($_REQUEST['cP']) or isset($_REQUEST['cU']) or isset($_REQUEST['cC']) or isset($_POST['cL'])) and $_SESSION['uid'] > 1) {
	saveLastSel($calID); //save last selected cP, cU, cC, cL
}
