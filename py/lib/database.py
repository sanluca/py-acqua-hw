...
class Mydata():
    def __init__(self):
        ...
        if db_is_new:            
            ...
            conn.execute("create table if not exists pwm2 (Id INTEGER PRIMARY KEY, period INT, duty_cycle INT, enable INT, start FLOAT, stop FLOAT,mezza FLOAT,onn INT);")
            conn.execute("create table if not exists pwm3 (Id INTEGER PRIMARY KEY, period INT, duty_cycle INT, enable INT, start FLOAT, stop FLOAT,mezza FLOAT,onn INT);")
            ...
            conn.execute("insert into pwm2 (period,duty_cycle,enable,start,stop,mezza,onn) values (%d ,%d,%d,%f,%f,%f,%d)" %(1000000,100000,1,0,0,0,0))
            conn.execute("insert into pwm3 (period,duty_cycle,enable,start,stop,mezza,onn) values (%d ,%d,%d,%f,%f,%f,%d)" %(1000000,100000,1,0,0,0,0))
            conn.execute("insert into calph (offset, range) values ('%f' , '%f')" %(0,0))
