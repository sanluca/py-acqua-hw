...
function done(action) { //close window and refresh calendar (action: c = close; r = reload opener)
	if (action.indexOf('r') > -1) { window.opener.location = window.opener.location.href; }
...
function xxable(clName,value) { //toggle dis/enable input elements with class == clName
	var elms = $C(clName);
