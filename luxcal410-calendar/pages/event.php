...
function addUrlImgTags($html) {
	global $baseUrl, $set, $rxURL, $rxIMG;

	$html = preg_replace_callback($rxURL,create_function('$m','return " <a class=\'link\' href=\'".(!empty($m[1])?$m[1]:"http://").$m[2]."\' target=\'_blank\'>".(!empty($m[3])?$m[3]:$m[2])."</a>";'),$html); //create HTML links
	$baseUrl = rtrim(preg_replace('~\?cal=.*$~','',$set['calendarUrl']),'/').'/';
	$html = preg_replace_callback($rxIMG,create_function('$m','global $baseUrl; return " <img src=\'".$baseUrl.(!empty($m[2])?$m[2]:\'thumbnails/\')."{$m[3]}\' alt=\'{$m[3]}\'>";'),$html); //create image links
	return $html;
}

//remove URL and IMG tags
function remUrlImgTags($html) {
	global $rxULink, $rxIMGTags;

	$html = preg_replace_callback($rxULink,create_function('$m','return $m[1].($m[2]!=$m[3]?" [".$m[3]."]":"");'),$html); //remove URL tags
	$html = preg_replace($rxIMGTags,"$1 ",$html); //remove IMG tags
	return $html;
}
...
if ($mode == 'edit' and !$refresh) { //show/edit event
	$stH = stPrep("
		SELECT e.*,c.name,c.approve,c.color,c.bgColor,u.name
		FROM events e
	...
	$tit = isset($_POST['tit']) ? strip_tags(htmlspecialchars_decode(trim($_POST['tit'])),'<b><i><u><s><center>') : '';
	$ven = isset($_POST['ven']) ? strip_tags(htmlspecialchars_decode(trim($_POST['ven'])),'<b><i><u><s><center>') : '';
	$des = isset($_POST['des']) ? strip_tags(htmlspecialchars_decode(trim($_POST['des'])),'<a><b><i><u><s><img>') : '';
	$xf1 = isset($_POST['xf1']) ? strip_tags(htmlspecialchars_decode(trim($_POST['xf1'])),'<b><i><u><s><center>') : '';
	$xf2 = isset($_POST['xf2']) ? strip_tags(htmlspecialchars_decode(trim($_POST['xf2'])),'<b><i><u><s><center>') : '';
	...
	$stH = stPrep("SELECT name,approve FROM categories WHERE ID = ? LIMIT 1");
	stExec($stH,array($cid));
		...
		do {
			...
			if ($mode == "add_exe") { //add new event
				...
				if ($editN != 1) { //update the series
					...
					stExec($stH,array($pri,$tit,$ven,$desHtml,$xf1Html,$xf2Html,$cid,$uid,$uname,$apd,$not,$nml,$sDate,$eDate,$sTime,$eTime,$r_t,$r_i,$r_p,$r_m,$runtil,$todayDT,$eid));
